...
class DuplicateDetectorApp:
    ...
    def find_duplicates(self) -> Tuple[Optional[Dict], str]:
        ...
        try:
            # Фиксированная точность 85% (для векторного метода)
            self.detector.similarity_threshold = 0.85
            ...
            if self.duplicate_groups:
                ...
                report += f"⚙️ Метод: **Векторизация + косинусное расстояние (85%)**\n"
                report += f"🔬 **Улучшенный алгоритм** - меньше ложных срабатываний"
                ...
                return (
                    ...
                    f"✅ **Дубликаты не найдены!**\n\nВсе {len(self.current_df)} записей уникальны при использовании векторного метода (85%)"
                )
